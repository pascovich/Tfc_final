...
$pres=$db->prepare("SELECT * FROM profile_candidat");
$pres->execute();
$date=date('Y-m-d');
...
<head>
  ...
  <title>Vote | candidatures retenues</title>
...
portant agrément de quelques
 Etablissements d’Enseignement Sup et Univ -->
    ...
    <hr>
    <h4 style="text-align: center;">Liste des candidatures retenues</h4>
    <!-- Table row -->
    <div class="row">
      <div class="col-xs-12 table-responsive">
        <table class="table table-striped">
          ...
          <tr>
            <td><img src="../media/images/<?= $electeur['IMAGE']; ?>" width="25%"></td>
            <td><?=$electeur['NOM'];?></td>
            ...
            <!-- <td><?=$electeur['SEXE'];?></td> -->
            <td><?=$electeur['NOMVOTE'];?></td>
            <td><?=$electeur['SLOGAN'];?></td>
            <td><?=$electeur['NUMVOTE'];?></td>
          </tr>
          <?php endwhile; ?>
          </tbody>
        </table>
